...
void loop()
{
  if(sbus.read(&channels[0], &failSafe, &lostFrame))
  {
      ...
      Joystick.button(3, map(channels[SWA - 1], 240, 1807, 0, 2) == 2 ? 1 : 0); // SWA HIGH = button 3
      
      Joystick.button(4, map(channels[SWB - 1], 240, 1807, 0, 2) == 0 ? 1 : 0); // SWB LOW = button 4
      Joystick.button(5, map(channels[SWB - 1], 240, 1807, 0, 2) == 1 ? 1 : 0); // SWB MID = button 5
      Joystick.button(6, map(channels[SWB - 1], 240, 1807, 0, 2) == 2 ? 1 : 0); // SWB HIGH = button 6
      
      Joystick.button(7, map(channels[SWC - 1], 240, 1807, 0, 1) == 0 ? 1 : 0); // SWC LOW = button 7
      Joystick.button(8, map(channels[SWC - 1], 240, 1807, 0, 1) == 1 ? 1 : 0); // SWC HIGH = button 8
